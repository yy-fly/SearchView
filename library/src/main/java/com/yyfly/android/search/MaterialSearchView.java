...
public class MaterialSearchView extends FrameLayout {
    ...
    /**
     * whether to keep the search history or not.
     */
    ...
    /**
     * The bar at the top of the MaterialSearchView containing the EditText and ImageButtons.
     */
    ...
    private void init() {
        // Inflate view
        LayoutInflater.from(mContext).inflate(R.layout.sv_view_search, this, true);
        ...
        historyLabel = (TextView) mRoot.findViewById(R.id.historyLabel);
        clearAll = (TextView) mRoot.findViewById(R.id.clearAll);
        mSuggestionsListView = (ExpandListView) mRoot.findViewById(R.id.suggestion_list);
    ...
    private void initStyle(AttributeSet attributeSet, int defStyleAttribute) {
        ...
        if (typedArray != null) {
            ...
            if (typedArray.hasValue(R.styleable.MaterialSearchView_searchBarBackground)) {
                setSearchBarColor(typedArray.getResourceId(
                        R.styleable.MaterialSearchView_searchBarBackground,
            ...
            if (typedArray.hasValue(R.styleable.MaterialSearchView_android_textColorHint)) {
                setHintTextColor(typedArray.getColor(R.styleable.MaterialSearchView_android_textColorHint,
                        ContextCompat.getColor(mContext, R.color.sv_color_gray_50)));
            }
            ...
            if (typedArray.hasValue(R.styleable.MaterialSearchView_searchVoiceIcon)) {
                setVoiceIcon(typedArray.getResourceId(
                        ...
                        R.drawable.sv_ic_action_voice_search)
                );
            }
            ...
            if (typedArray.hasValue(R.styleable.MaterialSearchView_searchCloseIcon)) {
                setClearIcon(typedArray.getResourceId(
                        ...
                        R.drawable.sv_ic_action_navigation_close)
                );
            }
            ...
            if (typedArray.hasValue(R.styleable.MaterialSearchView_searchBackIcon)) {
                setBackIcon(typedArray.getResourceId(
                        ...
                        R.drawable.sv_ic_action_navigation_arrow_back)
                );
            }
            ...
            if (typedArray.hasValue(R.styleable.MaterialSearchView_voiceHintPrompt)) {
                ...
                setVoiceHintPrompt(mContext.getString(R.string.sv_str_voice_hint_prompt));
            }
    ...
    /**
     * Preforms necessary initializations on the MaterialSearchView.
     */
    ...
    public void displayClearAll(boolean isShow) {
        if (isShow) {
            if (!historyLabel.isShown()) {
                historyLabel.setVisibility(VISIBLE);
                clearAll.setVisibility(VISIBLE);
            }
        } else {
            historyLabel.setVisibility(GONE);
    ...
    /**
     * Displays the MaterialSearchView.
     */
...
//                refreshAdapterCursor();
    ...
    /**
     * Sets whether the MSV should be keeping track of the submitted queries or not.
     *
    ...
    /**
     * Sets the background of the MaterialSearchView.
     *
    ...
    /**
     * Sets the background color of the MaterialSearchView.
     *
    ...
    public void setSearchBarColor(int resource) {
        // Set background color of search bar.
        if (resource > 0) {
            mSearchBar.setBackgroundResource(resource);
        }
    }
    ...
    /**
     * Sets the bar height if preferred to not use the existing actionbar height value
     *
    ...
    public void setVoiceHintPrompt(final String hintPrompt) {
        if (!TextUtils.isEmpty(hintPrompt)) {
            ...
            mHintPrompt = mContext.getString(R.string.sv_str_voice_hint_prompt);
        }
    }
    ...
    /**
     * Gets the current text on the MaterialSearchView, if any. Returns an empty String if no text is available.
     *
    ...
    public interface OnQueryTextListener {
         ...
         * @return True when the query is handled by the listener, false to let the MaterialSearchView handle the default case.
         */
    ...
    /**
     * Interface that handles the opening and closing of the MaterialSearchView.
     */
